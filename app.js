...
const AVATARS = [
  'https://models.readyplayer.me/65a8dba831b23abb4f401bae.glb?quality=medium&morphTargets=ARKit,Oculus%20Visemes&textureAtlas=none&useDracoMeshCompression=false',
  'https://models.readyplayer.me/638df48b65dcf4dcd8c0ad10.glb?quality=medium&morphTargets=ARKit,Oculus%20Visemes&textureAtlas=none&useDracoMeshCompression=false'
];
...
const cats = {
  ...
  Nose: ['nose','sneer'],
  Mouth: ['mouth','jaw','lip','smile','pucker','funnel'],
  Cheek: ['cheek']
};
...
async function loadAvatar(){
  for(let i=0;i<AVATARS.length;i++){
    loaderText.textContent = `מנסה לטעון morphable head ${i+1}/${AVATARS.length}...`;
    try{
      const gltf = await new Promise((resolve,reject)=>{
        const l = new GLTFLoader(); l.setMeshoptDecoder(MeshoptDecoder);
        l.load(AVATARS[i], resolve, undefined, reject);
      });
  ...
  loaderText.innerHTML = '<span class="err">לא הצלחתי לטעון את מודל ה־morph. זה אומר שהמודל/שרת חסם טעינה. נצטרך להעלות GLB משלנו לריפו.</span>';
}

function setupModel(root){
  modelRoot.clear(); morphMeshes=[]; morphNames=[];
  root.traverse(o=>{
    if(o.isMesh){
      if(o.material){ const mats = Array.isArray(o.material)?o.material:[o.material]; mats.forEach(m=>{m.roughness=.72; m.metalness=0;}); }
      if(o.morphTargetDictionary && o.morphTargetInfluences){
        morphMeshes.push(o); Object.keys(o.morphTargetDictionary).forEach(k=>morphNames.push(k));
      }
    }
  });
  ...
  // normalize full avatar as bust/head study
  ...
  root.position.x -= center.x; root.position.z -= center.z; root.position.y -= box.min.y + 1.55;
  box = new THREE.Box3().setFromObject(root); box.getSize(size);

  const headY = box.max.y - size.y * .15;
  const headH = size.y * .24;
  buildLoomisGuides(headY, headH);
  setEdges(true);
  buildMorphSliders();
  setCamera('front');
  statusEl.innerHTML = morphNames.length ? `<span class="ok">נמצאו ${morphNames.length} morph targets אמיתיים. הקווים הירוקים/כחולים הם אובייקטים תלת־ממדיים באותה סצנה וזזים עם הראש.</span>` : '<span class="err">המודל נטען אבל לא נמצאו morph targets.</span>';
}
...
function buildLoomisGuides(y, h){
  ...
  // cranial ball rings and side plane
  ...
  // front center and thirds on face plane
  ...
  // ear range bracket brow to nose
...
function ellipse(a,b,x,z,rot,y,material,plane='xy',dash=false){
  const pts=[]; for(let i=0;i<=96;i++){ const t=Math.PI*2*i/96; let px=x,py=y,pz=z; if(plane==='xy'){px=x+Math.cos(t)*a; py=y+Math.sin(t)*b; pz=z;} if(plane==='xz'){px=x+Math.cos(t)*a; py=y; pz=z+Math.sin(t)*b;} if(plane==='zy'){px=x; py=y+Math.sin(t)*b; pz=z+Math.cos(t)*a;} pts.push(new THREE.Vector3(px,py,pz)); }
  const obj=new THREE.Line(new THREE.BufferGeometry().setFromPoints(pts), material); if(dash){obj.material=material.clone(); obj.material.opacity=.75;} return obj;
}
function faceArc(rx, yy, cy, front, material){
  const pts=[]; for(let i=0;i<=40;i++){ const u=-1+2*i/40; const x=u*rx; const z=front - Math.abs(u)*.035; pts.push(new THREE.Vector3(x,yy,z)); } return new THREE.Line(new THREE.BufferGeometry().setFromPoints(pts), material);
}
...
function setCamera(type){ const t=new THREE.Vector3(0,.95,0), d=2.65; const map={front:[0,t.y,d],threeq:[-1.25,t.y+.05,2.15],profile:[-d,t.y,.03],up:[0,t.y+.75,d*.75],down:[0,t.y-.72,d*.75]}; camera.position.set(...(map[type]||map.front)); controls.target.copy(t); controls.update(); }
